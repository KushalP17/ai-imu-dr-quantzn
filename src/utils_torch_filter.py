import torch
import matplotlib.pyplot as plt
import numpy as np
import os
import time
from termcolor import cprint
from utils_numpy_filter import NUMPYIEKF
from utils import prepare_data

from quantization import (
    QuantizedConv1d,
    QuantizedLinear,
    DequantizeTanh,
    record_activation_range,
)

REPLICATION_PAD_SIZE = 4

# ---------------------------------------------------------------------------
# Global device: use CUDA if available, otherwise fall back to CPU.
# All inference-time tensors and model parameters will be placed here.
# ---------------------------------------------------------------------------
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")


class InitProcessCovNet(torch.nn.Module):

        def __init__(self):
            super(InitProcessCovNet, self).__init__()

            self.beta_process = 3*torch.ones(2).double()
            self.beta_initialization = 3*torch.ones(2).double()

            self.factor_initial_covariance = torch.nn.Linear(1, 6, bias=False).double()
            """parameters for initializing covariance"""
            self.factor_initial_covariance.weight.data[:] /= 10

            self.factor_process_covariance = torch.nn.Linear(1, 6, bias=False).double()
            """parameters for process noise covariance"""
            self.factor_process_covariance.weight.data[:] /= 10
            self.tanh = torch.nn.Tanh()

        def forward(self, iekf):
            return

        def init_cov(self, iekf):
            # Create the constant input on the same device as the network weights
            device = next(self.parameters()).device
            alpha = self.factor_initial_covariance(
                torch.ones(1, device=device).double()).squeeze()
            beta = 10**(self.tanh(alpha))
            return beta

        def init_processcov(self, iekf):
            # Create the constant input on the same device as the network weights
            device = next(self.parameters()).device
            alpha = self.factor_process_covariance(
                torch.ones(1, device=device).double())
            beta = 10**(self.tanh(alpha))
            return beta


class MesNet(torch.nn.Module):
        def __init__(self):
            super(MesNet, self).__init__()
            self.beta_measurement = 3*torch.ones(2).double()
            self.tanh = torch.nn.Tanh()

            self.cov_net = torch.nn.Sequential(torch.nn.Conv1d(6, 32, 5),
                       torch.nn.ReplicationPad1d(REPLICATION_PAD_SIZE),
                       torch.nn.ReLU(),
                       torch.nn.Dropout(p=0.5),
                       torch.nn.Conv1d(32, 32, 5, dilation=3),
                       torch.nn.ReplicationPad1d(REPLICATION_PAD_SIZE),
                       torch.nn.ReLU(),
                       torch.nn.Dropout(p=0.5),
                       ).double()
            "CNN for measurement covariance"
            self.cov_lin = torch.nn.Sequential(torch.nn.Linear(32, 2),
                                              torch.nn.Tanh(),
                                              ).double()
            self.cov_lin[0].bias.data[:] /= 100
            self.cov_lin[0].weight.data[:] /= 100

        def forward(self, u, iekf):
            # beta_measurement and cov0_measurement must live on the same device
            # as the input u. We move them lazily here so the forward pass works
            # correctly regardless of which device u is on.
            device = u.device
            beta = self.beta_measurement.to(device)
            y_cov = self.cov_net(u).transpose(0, 2).squeeze()
            z_cov = self.cov_lin(y_cov)
            z_cov_net = beta.unsqueeze(0) * z_cov
            measurements_covs = (iekf.cov0_measurement.unsqueeze(0) * (10**z_cov_net))
            return measurements_covs


def _safe_set_buffer(module, name, tensor):
    """
    Register `tensor` as an nn.Module buffer when the Module infrastructure is
    fully initialised (i.e. `_buffers` exists on `module`), otherwise fall back
    to a plain attribute assignment.

    TORCHIEKF uses multiple inheritance (nn.Module + NUMPYIEKF).  The MRO means
    NUMPYIEKF.__init__ and its call to set_param_attr execute before
    torch.nn.Module.__init__ finishes, so `_buffers` does not exist on the very
    first call.  Three cases are handled:

      1. nn.Module not yet init'd  -> plain setattr (no _buffers dict yet)
      2. Buffer already registered -> overwrite _buffers dict directly
      3. Plain instance attr exists (leftover from case 1) -> delete it, then
         call register_buffer so the tensor is properly tracked from this point on
    """
    if not hasattr(module, '_buffers'):
        # nn.Module not yet initialised: plain assignment is all we can do.
        setattr(module, name, tensor)
    elif name in module._buffers:
        # Already a registered buffer: overwrite value without re-registering.
        module._buffers[name] = tensor
    else:
        # Remove any plain instance attribute of the same name (from case 1 above)
        # before calling register_buffer, which would raise KeyError otherwise.
        if name in module.__dict__:
            del module.__dict__[name]
        module.register_buffer(name, tensor)


class TORCHIEKF(torch.nn.Module, NUMPYIEKF):
    # NOTE: no class-level Id* tensor attributes here.
    # They used to live as class attributes but that causes register_buffer to
    # raise "attribute already exists" because PyTorch finds them on the class
    # before finding them in _buffers.  They are now registered as instance
    # buffers in __init__ via _safe_set_buffer so .to(device) moves them.

    def __init__(self, parameter_class=None):
        # Initialise nn.Module FIRST so that _buffers exists when
        # NUMPYIEKF.__init__ calls set_param_attr below.
        torch.nn.Module.__init__(self)
        # NUMPYIEKF.__init__ calls set_param_attr() internally.  Because
        # nn.Module is already initialised above, _safe_set_buffer will
        # correctly register Q and cov0_measurement as proper buffers.
        NUMPYIEKF.__init__(self, parameter_class=None)

        # mean and standard deviation of inputs for normalisation
        self.u_loc = None
        self.u_std = None
        self.initprocesscov_net = InitProcessCovNet()
        self.mes_net = MesNet()

        # Register identity matrices as buffers so .to(device) moves them.
        # Use _safe_set_buffer because set_param_attr (called from
        # NUMPYIEKF.__init__ above) may have already set some of these as
        # plain instance attributes via the NUMPYIEKF class hierarchy.
        _safe_set_buffer(self, 'Id1', torch.eye(1).double())
        _safe_set_buffer(self, 'Id2', torch.eye(2).double())
        _safe_set_buffer(self, 'Id3', torch.eye(3).double())
        _safe_set_buffer(self, 'Id6', torch.eye(6).double())
        _safe_set_buffer(self, 'IdP', torch.eye(21).double())

        if parameter_class is not None:
            self.filter_parameters = parameter_class()
            self.set_param_attr()

    def set_param_attr(self):
        # Copy scalar parameters from filter_parameters onto self
        attr_list = [a for a in dir(self.filter_parameters) if not a.startswith('__')
                     and not callable(getattr(self.filter_parameters, a))]
        for attr in attr_list:
            setattr(self, attr, getattr(self.filter_parameters, attr))

        Q = torch.diag(torch.Tensor([self.cov_omega, self.cov_omega, self.cov_omega,
                                     self.cov_acc, self.cov_acc, self.cov_acc,
                                     self.cov_b_omega, self.cov_b_omega, self.cov_b_omega,
                                     self.cov_b_acc, self.cov_b_acc, self.cov_b_acc,
                                     self.cov_Rot_c_i, self.cov_Rot_c_i, self.cov_Rot_c_i,
                                     self.cov_t_c_i, self.cov_t_c_i, self.cov_t_c_i])
                        ).double()
        # Use the safe helper so this works both when called from NUMPYIEKF.__init__
        # (before nn.Module.__init__) and when called later as a proper nn.Module.
        _safe_set_buffer(self, 'Q', Q)
        _safe_set_buffer(self, 'cov0_measurement',
                         torch.Tensor([self.cov_lat, self.cov_up]).double())

    def run(self, t, u, measurements_covs, v_mes, p_mes, N, ang0):

        dt = t[1:] - t[:-1]  # (s)
        Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P = self.init_run(dt, u, p_mes, v_mes,
                                       N, ang0)

        for i in range(1, N):
            Rot_i, v_i, p_i, b_omega_i, b_acc_i, Rot_c_i_i, t_c_i_i, P_i = \
                self.propagate(Rot[i-1], v[i-1], p[i-1], b_omega[i-1], b_acc[i-1], Rot_c_i[i-1],
                               t_c_i[i-1], P, u[i], dt[i-1])

            Rot[i], v[i], p[i], b_omega[i], b_acc[i], Rot_c_i[i], t_c_i[i], P = \
                self.update(Rot_i, v_i, p_i, b_omega_i, b_acc_i, Rot_c_i_i, t_c_i_i, P_i,
                            u[i], i, measurements_covs[i])
        return Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i

    def init_run(self, dt, u, p_mes, v_mes, N, ang0):
            Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i = \
                self.init_saved_state(dt, N, ang0)
            Rot[0] = self.from_rpy(ang0[0], ang0[1], ang0[2])
            v[0] = v_mes[0]
            P = self.init_covariance()
            return Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P

    def init_covariance(self):
        beta = self.initprocesscov_net.init_cov(self)
        P = torch.zeros(self.P_dim, self.P_dim).double()
        P[:2, :2] = self.cov_Rot0*beta[0]*self.Id2  # no yaw error
        P[3:5, 3:5] = self.cov_v0*beta[1]*self.Id2
        P[9:12, 9:12] = self.cov_b_omega0*beta[2]*self.Id3
        P[12:15, 12:15] = self.cov_b_acc0*beta[3]*self.Id3
        P[15:18, 15:18] = self.cov_Rot_c_i0*beta[4]*self.Id3
        P[18:21, 18:21] = self.cov_t_c_i0*beta[5]*self.Id3
        return P

    def init_saved_state(self, dt, N, ang0):
        Rot = dt.new_zeros(N, 3, 3)
        v = dt.new_zeros(N, 3)
        p = dt.new_zeros(N, 3)
        b_omega = dt.new_zeros(N, 3)
        b_acc = dt.new_zeros(N, 3)
        Rot_c_i = dt.new_zeros(N, 3, 3)
        t_c_i = dt.new_zeros(N, 3)
        Rot_c_i[0] = torch.eye(3).double()
        return Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i

    def propagate(self, Rot_prev, v_prev, p_prev, b_omega_prev, b_acc_prev, Rot_c_i_prev,
                  t_c_i_prev, P_prev, u, dt):
        Rot_prev = Rot_prev.clone()
        acc_b = u[3:6] - b_acc_prev
        acc = Rot_prev.mv(acc_b) + self.g
        v = v_prev + acc * dt
        p = p_prev + v_prev.clone() * dt + 1/2 * acc * dt**2

        omega = (u[:3] - b_omega_prev)*dt
        Rot = Rot_prev.mm(self.so3exp(omega))

        b_omega = b_omega_prev
        b_acc = b_acc_prev
        Rot_c_i = Rot_c_i_prev.clone()
        t_c_i = t_c_i_prev

        P = self.propagate_cov(P_prev, Rot_prev, v_prev, p_prev, b_omega_prev, b_acc_prev,
                               u, dt)
        return Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P

    def propagate_cov(self, P, Rot_prev, v_prev, p_prev, b_omega_prev, b_acc_prev, u, dt):

        F = P.new_zeros(self.P_dim, self.P_dim)
        G = P.new_zeros(self.P_dim, self.Q.shape[0])
        Q = self.Q.clone()
        F[3:6, :3] = self.skew(self.g)
        F[6:9, 3:6] = self.Id3
        G[3:6, 3:6] = Rot_prev
        F[3:6, 12:15] = -Rot_prev
        v_skew_rot = self.skew(v_prev).mm(Rot_prev)
        p_skew_rot = self.skew(p_prev).mm(Rot_prev)
        G[:3, :3] = Rot_prev
        G[3:6, :3] = v_skew_rot
        G[6:9, :3] = p_skew_rot
        F[:3, 9:12] = -Rot_prev
        F[3:6, 9:12] = -v_skew_rot
        F[6:9, 9:12] = -p_skew_rot
        G[9:12, 6:9] = self.Id3
        G[12:15, 9:12] = self.Id3
        G[15:18, 12:15] = self.Id3
        G[18:21, 15:18] = self.Id3

        F = F * dt
        G = G * dt
        F_square = F.mm(F)
        F_cube = F_square.mm(F)
        Phi = self.IdP + F + 1/2*F_square + 1/6*F_cube
        P_new = Phi.mm(P + G.mm(Q).mm(G.t())).mm(Phi.t())
        return P_new

    def update(self, Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P, u, i, measurement_cov):
        # orientation of body frame
        Rot_body = Rot.mm(Rot_c_i)
        # velocity in imu frame
        v_imu = Rot.t().mv(v)
        omega = u[:3] - b_omega
        # velocity in body frame
        v_body = Rot_c_i.t().mv(v_imu) + self.skew(t_c_i).mv(omega)
        Omega = self.skew(omega)
        # Jacobian in car frame
        H_v_imu = Rot_c_i.t().mm(self.skew(v_imu))
        H_t_c_i = self.skew(t_c_i)

        H = P.new_zeros(2, self.P_dim)
        H[:, 3:6] = Rot_body.t()[1:]
        H[:, 15:18] = H_v_imu[1:]
        H[:, 9:12] = H_t_c_i[1:]
        H[:, 18:21] = -Omega[1:]
        r = - v_body[1:]
        R = torch.diag(measurement_cov)

        Rot_up, v_up, p_up, b_omega_up, b_acc_up, Rot_c_i_up, t_c_i_up, P_up = \
            self.state_and_cov_update(Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P, H, r, R)
        return Rot_up, v_up, p_up, b_omega_up, b_acc_up, Rot_c_i_up, t_c_i_up, P_up

    @staticmethod
    def state_and_cov_update(Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P, H, r, R):
        S = H.mm(P).mm(H.t()) + R
        Kt, _ = torch.gesv(P.mm(H.t()).t(), S)
        K = Kt.t()
        dx = K.mv(r.view(-1))

        dR, dxi = TORCHIEKF.sen3exp(dx[:9])
        dv = dxi[:, 0]
        dp = dxi[:, 1]
        Rot_up = dR.mm(Rot)
        v_up = dR.mv(v) + dv
        p_up = dR.mv(p) + dp

        b_omega_up = b_omega + dx[9:12]
        b_acc_up = b_acc + dx[12:15]

        dR = TORCHIEKF.so3exp(dx[15:18])
        Rot_c_i_up = dR.mm(Rot_c_i)
        t_c_i_up = t_c_i + dx[18:21]

        I_KH = TORCHIEKF.IdP - K.mm(H)
        P_upprev = I_KH.mm(P).mm(I_KH.t()) + K.mm(R).mm(K.t())
        P_up = (P_upprev + P_upprev.t())/2
        return Rot_up, v_up, p_up, b_omega_up, b_acc_up, Rot_c_i_up, t_c_i_up, P_up

    @staticmethod
    def skew(x):
        # x.new_tensor ensures the result lives on the same device as x.
        return x.new_tensor([
            [ 0,    -x[2],  x[1]],
            [ x[2],  0,    -x[0]],
            [-x[1],  x[0],  0   ]
        ])

    @staticmethod
    def rot_from_2_vectors(v1, v2):
        """ Returns a Rotation matrix between vectors 'v1' and 'v2'    """
        v1 = v1/torch.norm(v1)
        v2 = v2/torch.norm(v2)
        v = torch.cross(v1, v2)
        cosang = v1.matmul(v2)
        sinang = torch.norm(v)
        Rot = TORCHIEKF.Id3 + TORCHIEKF.skew(v) + \
              TORCHIEKF.skew(v).mm(TORCHIEKF.skew(v))*(1-cosang)/(sinang**2)
        return Rot

    @staticmethod
    def sen3exp(xi):
        phi = xi[:3]
        angle = torch.norm(phi)

        # Near |phi|==0, use first order Taylor expansion
        if isclose(angle, 0.):
            # xi.new_tensor ensures skew matrices land on the same device as xi.
            skew_phi = xi.new_tensor([
                [ 0,      -phi[2],  phi[1]],
                [ phi[2],  0,      -phi[0]],
                [-phi[1],  phi[0],  0     ]
            ])
            J = TORCHIEKF.Id3 + 0.5 * skew_phi
            Rot = TORCHIEKF.Id3 + skew_phi
        else:
            axis = phi / angle
            skew_axis = xi.new_tensor([
                [ 0,       -axis[2],  axis[1]],
                [ axis[2],  0,       -axis[0]],
                [-axis[1],  axis[0],  0      ]
            ])
            s = torch.sin(angle)
            c = torch.cos(angle)

            J = (s / angle) * TORCHIEKF.Id3 + (1 - s / angle) * TORCHIEKF.outer(axis, axis)\
                   + ((1 - c) / angle) * skew_axis
            Rot = c * TORCHIEKF.Id3 + (1 - c) * TORCHIEKF.outer(axis, axis) \
                 + s * skew_axis

        x = J.mm(xi[3:].view(-1, 3).t())
        return Rot, x

    @staticmethod
    def so3exp(phi):
        angle = phi.norm()

        # Near phi==0, use first order Taylor expansion
        if isclose(angle, 0.):
            # phi.new_tensor ensures the result is on the same device as phi.
            skew_phi = phi.new_tensor([
                [ 0,      -phi[2],  phi[1]],
                [ phi[2],  0,      -phi[0]],
                [-phi[1],  phi[0],  0     ]
            ])
            Xi = TORCHIEKF.Id3 + skew_phi
            return Xi
        axis = phi / angle
        skew_axis = phi.new_tensor([
            [ 0,       -axis[2],  axis[1]],
            [ axis[2],  0,       -axis[0]],
            [-axis[1],  axis[0],  0      ]
        ])
        c = angle.cos()
        s = angle.sin()
        Xi = c * TORCHIEKF.Id3 + (1 - c) * TORCHIEKF.outer(axis, axis) \
             + s * skew_axis
        return Xi

    @staticmethod
    def outer(a, b):
        ab = a.view(-1, 1)*b.view(1, -1)
        return ab

    @staticmethod
    def so3left_jacobian(phi):
        angle = torch.norm(phi)

        # Near |phi|==0, use first order Taylor expansion
        if isclose(angle, 0.):
            skew_phi = phi.new_tensor([
                [ 0,      -phi[2],  phi[1]],
                [ phi[2],  0,      -phi[0]],
                [-phi[1],  phi[0],  0     ]
            ])
            return TORCHIEKF.Id3 + 0.5 * skew_phi

        axis = phi / angle
        skew_axis = phi.new_tensor([
            [ 0,       -axis[2],  axis[1]],
            [ axis[2],  0,       -axis[0]],
            [-axis[1],  axis[0],  0      ]
        ])
        s = torch.sin(angle)
        c = torch.cos(angle)

        return (s / angle) * TORCHIEKF.Id3 \
               + (1 - s / angle) * TORCHIEKF.outer(axis, axis) + ((1 - c) / angle) * skew_axis

    @staticmethod
    def to_rpy(Rot):
        """Convert a rotation matrix to RPY Euler angles."""

        pitch = torch.atan2(-Rot[2, 0], torch.sqrt(Rot[0, 0]**2 + Rot[1, 0]**2))

        if isclose(pitch, np.pi / 2.):
            yaw = pitch.new_zeros(1)
            roll = torch.atan2(Rot[0, 1], Rot[1, 1])
        elif isclose(pitch, -np.pi / 2.):
            yaw = pitch.new_zeros(1)
            roll = -torch.atan2(Rot[0, 1],  Rot[1, 1])
        else:
            sec_pitch = 1. / pitch.cos()
            yaw = torch.atan2(Rot[1, 0] * sec_pitch, Rot[0, 0] * sec_pitch)
            roll = torch.atan2(Rot[2, 1] * sec_pitch, Rot[2, 2] * sec_pitch)
        return roll, pitch, yaw

    @staticmethod
    def from_rpy(roll, pitch, yaw):
        """Form a rotation matrix from RPY Euler angles."""
        return TORCHIEKF.rotz(yaw).mm(TORCHIEKF.roty(pitch).mm(TORCHIEKF.rotx(roll)))

    @staticmethod
    def rotx(t):
        """Rotation about the x-axis."""
        c = torch.cos(t)
        s = torch.sin(t)
        return t.new([[1,  0,  0],
                      [0,  c, -s],
                      [0,  s,  c]])

    @staticmethod
    def roty(t):
        """Rotation about the y-axis."""
        c = torch.cos(t)
        s = torch.sin(t)
        return t.new([[c,  0,  s],
                      [0,  1,  0],
                      [-s, 0,  c]])

    @staticmethod
    def rotz(t):
        """Rotation about the z-axis."""
        c = torch.cos(t)
        s = torch.sin(t)
        return t.new([[c, -s,  0],
                      [s,  c,  0],
                      [0,  0,  1]])

    @staticmethod
    def normalize_rot(rot):
        # pytorch SVD seems to be inaccurate, so just move to numpy immediately
        U, _, V = torch.svd(rot)
        S = torch.eye(3).double()
        S[2, 2] = torch.det(U) * torch.det(V)
        return U.mm(S).mm(V.t())

    def forward_nets(self, u):
        u_n = self.normalize_u(u).t().unsqueeze(0)
        u_n = u_n[:, :6]
        measurements_covs = self.mes_net(u_n, self)
        return measurements_covs

    def normalize_u(self, u):
        return (u - self.u_loc) / self.u_std

    def get_normalize_u(self, dataset):
        # Always place u_loc / u_std on the same device as the model weights.
        device = next(self.parameters()).device
        self.u_loc = dataset.normalize_factors['u_loc'].double().to(device)
        self.u_std = dataset.normalize_factors['u_std'].double().to(device)

    def set_Q(self):
        """Update the process noise covariance."""
        device = self.Id3.device

        Q_base = torch.diag(torch.Tensor([
            self.cov_omega, self.cov_omega, self.cov_omega,
            self.cov_acc, self.cov_acc, self.cov_acc,
            self.cov_b_omega, self.cov_b_omega, self.cov_b_omega,
            self.cov_b_acc, self.cov_b_acc, self.cov_b_acc,
            self.cov_Rot_c_i, self.cov_Rot_c_i, self.cov_Rot_c_i,
            self.cov_t_c_i, self.cov_t_c_i, self.cov_t_c_i,
        ])).double().to(device)
        _safe_set_buffer(self, 'Q', Q_base)

        beta = self.initprocesscov_net.init_processcov(self)
        Q_new = torch.zeros(self.Q.shape[0], self.Q.shape[0]).double().to(device)
        Q_new[:3, :3] = self.cov_omega * beta[0] * self.Id3
        Q_new[3:6, 3:6] = self.cov_acc * beta[1] * self.Id3
        Q_new[6:9, 6:9] = self.cov_b_omega * beta[2] * self.Id3
        Q_new[9:12, 9:12] = self.cov_b_acc * beta[3] * self.Id3
        Q_new[12:15, 12:15] = self.cov_Rot_c_i * beta[4] * self.Id3
        Q_new[15:18, 15:18] = self.cov_t_c_i * beta[5] * self.Id3
        _safe_set_buffer(self, 'Q', Q_new)

    def load(self, args, dataset):
        self.get_normalize_u(dataset)
        path_iekf = os.path.join(args.path_temp, "iekfnets.p")
        if os.path.isfile(path_iekf):
            # map_location=DEVICE loads weights directly onto the target device,
            # avoiding an unnecessary CPU->GPU copy after the fact.
            mondict = torch.load(path_iekf, map_location=DEVICE)

            w = mondict["mes_net.cov_net.4.weight"]
            b = mondict["mes_net.cov_net.4.bias"]

            w32 = 0.5 * (w[0::2] + w[1::2])
            b32 = 0.5 * (b[0::2] + b[1::2])

            mondict["mes_net.cov_net.4.weight"] = w32.clone()
            mondict["mes_net.cov_net.4.bias"] = b32.clone()

            self.load_state_dict(mondict, strict=False)

            # Move the entire model (parameters + registered buffers) to DEVICE.
            self.to(DEVICE)

            # u_loc / u_std are plain tensors, not parameters/buffers, so we
            # must move them manually after calling .to(DEVICE).
            if self.u_loc is not None:
                self.u_loc = self.u_loc.to(DEVICE)
                self.u_std = self.u_std.to(DEVICE)

            cprint("IEKF nets loaded onto {}".format(DEVICE), "green")

            if args.quantize:
                self.quantize(args, dataset)
        else:
            cprint("IEKF nets NOT loaded", "yellow")

    def quantize(self, args, dataset):
        input_activation = {}
        output_activation = {}

        for i in range(0, len(dataset.datasets)):
            dataset_name = dataset.dataset_name(i)
            if dataset_name not in dataset.odometry_benchmark.keys():
                continue
            print("Recoding activation range on sequence: " + dataset_name)
            _, _, _, _, u = prepare_data(args, dataset, dataset_name, i, to_numpy=True)
            u_t = torch.from_numpy(u).double().to(DEVICE)
            input_activation, output_activation = record_activation_range(
                self.mes_net, self.forward_nets, u_t
            )
            break

        if input_activation and output_activation:
            cprint("Activation range recorded", "green")

            conv_0 = QuantizedConv1d.build(
                self.mes_net.cov_net[0],
                "cov_net.0",
                "cov_net.2",
                input_activation,
                output_activation,
                pad_size=REPLICATION_PAD_SIZE,
                first_layer=True,
            )
            conv_4 = QuantizedConv1d.build(
                self.mes_net.cov_net[4],
                "cov_net.4",
                "cov_net.6",
                input_activation,
                output_activation,
                pad_size=REPLICATION_PAD_SIZE,
            )
            linear_0 = QuantizedLinear.build(
                self.mes_net.cov_lin[0],
                "cov_lin.0",
                input_activation,
                output_activation,
            )
            tahn_1 = DequantizeTanh(linear_0.output_scale, linear_0.output_zero_point)

            self.mes_net.cov_net = torch.nn.Sequential(conv_0, conv_4)
            self.mes_net.cov_lin = torch.nn.Sequential(linear_0, tahn_1)

            cprint("IEKF nets quantized", "green")
        else:
            cprint("Activation range NOT recorded", "yellow")
            cprint("IEKF nets NOT quantized", "yellow")


def isclose(mat1, mat2, tol=1e-10):
    return (mat1 - mat2).abs().lt(tol)


def prepare_filter(args, dataset):
    torch_iekf = TORCHIEKF()
    torch_iekf.load(args, dataset)
    torch_iekf = TORCHIEKF()

    # set dataset parameter
    torch_iekf.filter_parameters = args.parameter_class()
    torch_iekf.set_param_attr()
    if type(torch_iekf.g).__module__ == np.__name__:
        torch_iekf.g = torch.from_numpy(torch_iekf.g).double()

    # load model
    torch_iekf.load(args, dataset)
    torch_iekf.get_normalize_u(dataset)

    iekf = NUMPYIEKF(args.parameter_class)
    iekf.set_learned_covariance(torch_iekf)
    return iekf, torch_iekf
